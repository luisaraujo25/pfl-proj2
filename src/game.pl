...
game_cycle(gameState(TurnNumber, Board, player(human), P2)):-
    ...
    move(gameState(TurnNumber, Board, player(human), P2), Move, NewGameState),
...
game_cycle(gameState(TurnNumber, Board, player(computer), P2)):-
    ...
    move(gameState(TurnNumber, Board, player(computer), P2), Move, NewGameState), 
...
game_cycle(gameState(TurnNumber, Board, P1, player(human))):-
    ...
    move(gameState(TurnNumber, Board, P1, player(human)), Move, NewGameState), 
    display_game(NewGameState),
	get_code(_), %read code 10 (?)
    game_cycle(NewGameState).
...
game_cycle(gameState(TurnNumber, Board, P1, player(computer))):-
	...
	sleep(SleepTime),
	choose_move(gameState(TurnNumber, Board, _, _), 1, Move),
    move(gameState(TurnNumber, Board, P1, player(computer)), Move, NewGameState),
    write('Computer`s Turn'),
	display_game(NewGameState),
    game_cycle(NewGameState).
% 
...
playerAskMove(gameState(TurnNumber, Board, P1, P2), move(Xi, Yi, Xf, Yf)) :-
	...
	write('What piece you want to move? (write coordinates)'), nl, write('X: '), get_code(Xinput), write('Y: '), get_code(_), %read code 10 (?)
	get_code(Yinput),
	get_code(_), %read code 10 (?)
...
matchingColor(Color, Color, gameState(TurnNumber, Board, P1, P2), move(Xi, Yi, Xf, Yf)) :-
	write('Where do you want to move it to? (write coordinates)'), nl, write('X: '), get_code(Xfinput), write('Y: '), get_code(_), %read code 10 (?)
	get_code(Yfinput),  nl,
...
isDestinationValid(gameState(TurnNumber, Board, P1, P2), move(Xi, Yi, Xf, Yf)) :-
	...
	write('Not a valid move'), nl, get_code(_), %read code 10 (?)
	game_cycle(gameState(TurnNumber, Board, P1, P2)).
